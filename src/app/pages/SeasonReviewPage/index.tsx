...
export const SeasonReviewPage = props => {
  ...
  useEffect(() => {
    ...
    if (id !== '0') {
      ...
      fetch('https://localhost:5001/seasonReview/' + id, requestOptions)
        .then(response => {
          if (response.ok) {
  ...
  const save = event => {
    ...
    review.seasonId = props.location.state.seasonId;
    let requestOptions = {
    ...
    fetch('https://localhost:5001/seasonReview', requestOptions)
      .then(response => {
        if (response.ok) {
          if (id === '0') history.push('/season/' + review.seasonId);
        } else
          response
            .json()
  ...
  return !loading ? (
    <Container fluid>
      <Row className="justify-content-md-center" style={{ marginTop: '60px' }}>
        <Col lg={6}>
          <Form.Label>
            Review for {props.location.state.series} season{' '}
