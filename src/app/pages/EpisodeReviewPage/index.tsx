...
export const EpisodeReviewPage = props => {
  ...
  useEffect(() => {
    ...
    if (id !== '0') {
      ...
      fetch('https://localhost:5001/episodeReview/' + id, requestOptions)
        .then(response => {
          if (response.ok) {
  ...
  const save = event => {
    ...
    review.episodeId = props.location.state.episodeId;
    let requestOptions = {
  ...
  return !loading ? (
    <Container fluid>
      <Row className="justify-content-md-center" style={{ marginTop: '60px' }}>
        <Col lg={6}>
          <Form.Label>
            Review for {props.location.state.seriesTitle} season{' '}
            {props.location.state.seasonNumber} episode{' '}
